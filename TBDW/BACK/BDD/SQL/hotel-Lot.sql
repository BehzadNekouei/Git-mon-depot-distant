...
SELECT  `hot_nom`AS `le nom de l’hôtel`,
`hot_ville` AS `la ville`
FROM `hotel`;
-- 2 - Afficher la ville de résidence de Mr White Le résultat doit faire apparaître le nom, le prénom, et l'adresse du client
SELECT  `cli_nom` AS `le nom du client`,
`cli_prenom` AS `le prénom`,
`cli_adresse` AS `l'adresse du client`,
`cli_ville` AS `la ville`
FROM `client` 
WHERE `cli_nom`='White';
...
SELECT `sta_nom` AS `le nom de la station`,
`sta_altitude` AS  `l'altitude`
FROM `station` 
...
SELECT  `cha_numero` AS `le numéro de la chambre`,
`cha_capacite` AS `la capacité`
FROM `chambre` 
...
`hot_categorie` AS `la catégorie`,
`hot_ville` AS  `la ville`,
`cha_numero` AS `le numéro de la chambre`
FROM `hotel`, `chambre`
...
`hot_ville` AS  `la ville` ,
`cha_numero` AS `le numéro de la chambre`,
`cha_capacite` AS `sa capacité` 
FROM `hotel`, `chambre`  
WHERE `cha_hot_id`=`hot_id` AND `cha_capacite`>1;
...
SELECT `cli_nom` AS `le nom du client` ,
`hot_nom` AS `le nom de l’hôtel` , 
`res_date_debut` AS `la date de début du séjour` ,
 DATEDIFF(`res_date_fin`,`res_date_debut`) AS `la durée du séjour`
FROM `client`
JOIN `reservation`
ON `client`.`cli_id` = `reservation`.`res_cli_id`
JOIN `chambre`
...
SELECT DISTINCT(`hot_nom`) AS `la liste des hôtels pour lesquels Mr Squire a effectué une réservation`
FROM `reservation`
JOIN `chambre`
